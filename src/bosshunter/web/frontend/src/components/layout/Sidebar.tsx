...
export function Sidebar({ pendingReplies: pendingRepliesProp }: SidebarProps) {
  ...
  return <aside className="flex w-16 shrink-0 flex-col border-r border-card-border bg-white md:w-60">
    <div className="flex h-16 shrink-0 items-center justify-center border-b border-card-border md:justify-start md:px-5"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-primary text-white shadow-lg shadow-primary/20"><span className="text-sm font-black">BH</span></div><div className="hidden md:block"><div className="text-sm font-black tracking-tight text-foreground">BossHunter</div><div className="text-[11px] text-muted">v2.4.0 · 本地控制台</div></div></div></div>
    ...
    <div className="space-y-3 border-t border-card-border px-2 py-4 md:px-4"><a href={GITHUB_URL} target="_blank" rel="noreferrer" aria-label="BossHunter GitHub" className="relative flex items-center justify-center rounded-xl border border-card-border bg-[#FFFCFA] px-3 py-3 text-xs font-black text-foreground transition-colors hover:border-primary/60 hover:text-primary md:rounded-2xl"><Github className="h-4 w-4 shrink-0 md:absolute md:left-3" /><span className="mx-auto hidden items-center justify-center gap-2 md:flex"><span className="text-xl leading-none text-yellow-400">★</span>BossHunter</span></a><p className="hidden text-center text-[11px] leading-5 text-muted md:block">❤️ 欢迎 Star 支持维护 ❤️</p></div>
  </aside>
}
