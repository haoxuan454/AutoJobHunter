...
import { ArrowUpDown, MessageCircle, PauseCircle, RefreshCw, Trash2 } from 'lucide-react'
...
export default function ConversationsPage() {
  ...
  const sync = async () => {
    ...
    try {
      ...
      const failed = results.filter((item: { status?: string }) => item.status === 'error').length
      setNotice(failed ? `会话同步部分完成：${failed} 个平台读取失败` : '已完成已打开平台会话同步')
      await load()
    } catch (error) { setNotice(error instanceof Error ? error.message : '会话同步失败') } finally { setSyncing(false) }
  }
