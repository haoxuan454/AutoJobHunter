...
class BrowserRuntimeManagerTests(unittest.TestCase):
    ...
    def test_ensure_runtime_starts_builtin_runtime_when_auto_start_enabled(self, check_node, start_runtime, runtime_health):
        from bosshunter.browser.runtime import ensure_runtime

        check_node.return_value = {"available": True, "version": "v22.1.0"}
        runtime_health.side_effect = [None, None, {"status": "ok", "runtime": "bosshunter"}]
    ...
    @patch("bosshunter.browser.runtime.runtime_targets", return_value=None)
    @patch("bosshunter.browser.runtime.start_runtime")
    @patch("bosshunter.browser.runtime.check_node_available")
    @patch("bosshunter.browser.runtime.runtime_health")
    def test_ensure_runtime_does_not_require_chrome_connection(self, runtime_health, check_node, start_runtime, runtime_targets):
        from bosshunter.browser.runtime import ensure_runtime

        runtime_health.return_value = {"status": "ok", "runtime": "bosshunter", "connected": False}

        result = ensure_runtime({"browser": {"runtime": "builtin", "auto_start_proxy": True}})

        self.assertTrue(result)
        check_node.assert_not_called()
    ...
    def test_ensure_runtime_uses_next_free_port_when_default_has_non_bosshunter_service(
        ...
        runtime_health.side_effect = lambda browser: (
            {"status": "ok", "runtime": "web-access"}
            if browser["proxy_port"] == 3456
            else {"status": "ok", "runtime": "bosshunter"}
        )
