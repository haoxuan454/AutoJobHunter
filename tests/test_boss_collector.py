...
class BossCollectorUnitTests(TestCase):
    ...
    def test_list_script_targets_job_card_wrap(self):
        ...
        self.assertIn('a.job-info[href*="/job_detail/"]', JS_EXTRACT_LIST)
        self.assertIn(".sub-li-bottom .user-info .name", JS_EXTRACT_LIST)
...
class BossFreshSearchTests(TestCase):
    ...
    def test_repeated_runs_find_new_jobs_despite_old_word_and_page_checkpoints(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            urls += [call.args[1] for call in browser.navigate.call_args_list if "/web/geek/jobs?" in call.args[1]]
